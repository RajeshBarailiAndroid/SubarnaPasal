...
async function fetchJson(url, options = {}) {
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      throw new Error('Metal price API timed out. Try again in a moment.');
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
function isMetalApiConfigured() {
  ...
  if (provider === 'metals-api' || provider === 'goldapi' || provider === 'goldapi.io') {
    return hasValidApiKey();
  }
  // Unknown provider — fall back to free gold-api.com
  return true;
}
...
async function getLiveMetalRates(currency = 'USD') {
  if (!isMetalApiConfigured()) {
    const err = new Error(
      'Live metal API is not configured. Set METAL_PRICE_PROVIDER=gold-api in Vercel env, or add METAL_PRICE_API_KEY for goldapi/metals-api.'
    );
  ...
  const provider = getProvider();
  let data;
  if (provider === 'metals-api' && hasValidApiKey()) {
    data = await fetchFromMetalsApi();
  } else if ((provider === 'goldapi' || provider === 'goldapi.io') && hasValidApiKey()) {
