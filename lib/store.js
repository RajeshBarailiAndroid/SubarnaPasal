...
const { isMissingTableError, isMissingUserIdColumnError, missingTablesMessage, missingUserIdMessage } = require('./db-schema');
...
function defaultSettings() {
  return {
    ...
    itemCategories: ['Ring', 'Necklace', 'Bangle', 'Earring', 'Coin', 'Bar', 'Other'],
    rateHistory: [],
...
async function writeSupabaseStore(data, userId) {
  ...
  await syncTable(
    supabase,
    'customers',
    (data.customers || []).map((customer) => customerToRow(customer, userId)),
    userId
  );
}
